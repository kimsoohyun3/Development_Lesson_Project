...
public interface RecruitTeacherRepository extends JpaRepository<RecruitTeacher, Long> {
    ...
    // 게시물 리스트 조회
    @Query("select new project.lesson.dto.RecruitTeacherDto(rt.id, rt.title, rt.content) from RecruitTeacher rt")
    List<RecruitTeacherDto> findByPosts();

    // 게시물 ID로 게시물 단건 조회
    @Query("select new project.lesson.dto.RecruitTeacherDto(rt.id, rt.title, rt.content) from RecruitTeacher rt where rt.id = :postId")
